...
class TypeClassesSpec extends FunSuite {
    ...
    // type class INSTANCES: (usually people put these implicits inside another object "<something>Instances")
    implicit val dogBehavesLikeHuman = new BehavesLikeHuman[Dog] {
      def speak(dog: Dog): String = s"I'm a dog named: ${dog.name}"
    }

    // type class SYNTAX: (this is the interfaces that the consumer will use), usually people put these implicits inside another object "<something>Syntax"
